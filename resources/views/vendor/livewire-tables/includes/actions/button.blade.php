<a {{ $attributes->merge()
        ->class([
            // Tailwind compact
            'justify-center text-center items-center inline-flex space-x-1 rounded-md border shadow-sm px-2 py-1 text-xs font-medium focus:ring focus:ring-opacity-50' 
                => $isTailwind && ($attributes['default-styling'] ?? true),
            'focus:border-indigo-300 focus:ring-indigo-200' 
                => $isTailwind && ($attributes['default-colors'] ?? true),

            // Bootstrap compact
            'btn btn-sm btn-success' 
                => $isBootstrap && ($attributes['default-styling'] ?? true),
            '' 
