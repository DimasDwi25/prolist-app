...
@section('content')
    ...
    <div class="max-w-6xl mx-auto bg-white p-4 sm:p-6 lg:p-8 rounded-xl shadow border border-gray-200">

        {{-- Header --}}
        <div class="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-gray-800">📁 View Project</h2>
                <a href="{{ route('supervisor.project') }}"
                    class="text-sm text-gray-500 hover:text-blue-600 hover:underline block mt-1">
                    ← Back to Projects
                </a>
            </div>
            ...
            <div class="flex flex-wrap gap-2 justify-center md:justify-end">
                <a href="{{ route('project.edit', $project) }}"
                    class="inline-flex bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-2 rounded-md text-sm shadow transition">
                    ✏️ Edit Project
                </a>
                ...
                @if ($project->phc)
                    ...
                    <a href="{{ route('phc.show', $project->phc->id) }}"
                        class="inline-flex bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-md text-sm shadow transition">
                        👁️ View PHC
                    </a>
                @else
                    <a href="{{ route('phc', $project->id) }}"
                        class="inline-flex bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-md text-sm shadow transition">
                        ➕ Create PHC
                    </a>
                @endif

                <a href="{{ route('supervisor.projects.logs', $project->id) }}"
                    class="inline-flex bg-gray-700 hover:bg-gray-800 text-white px-3 py-2 rounded-md text-sm shadow transition">
                    📋 View Logs
                </a>
            </div>
        ...
        <div x-data="{ tab: 'info' }">
            <div class="border-b mb-4 overflow-x-auto">
                <nav class="flex space-x-4 text-sm font-medium text-gray-600 min-w-max">
            ...
            <div x-show="tab === 'quotation'" x-cloak>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-4">
        ...
        {{-- Project Logs --}}
        <div class="mt-8">
            <h3 class="text-lg sm:text-xl font-bold text-gray-700 mb-4">📜 Project Logs</h3>
            <div class="bg-white rounded-lg shadow border border-gray-200 p-4 overflow-x-auto">
                {{-- Desktop & Mobile (DataTable) --}}
