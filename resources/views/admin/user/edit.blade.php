...
@section('content')
    ...
    <form action="{{ route('user.update', $user) }}" method="POST" class="bg-white p-6 rounded shadow-md w-full max-w-xl">
        ...
        <div class="mb-4">
            <label class="block mb-2 text-gray-700">PIN</label>
            <input type="text" name="pin" class="w-full border px-4 py-2 rounded" value="{{ old('pin') }}"
                placeholder="Leave blank to keep current PIN">
            <p class="text-sm text-gray-500">Leave blank if you don't want to change the PIN.</p>
            @error('pin')
